...
class ImageWrapper
{
     ...
     * @var string
     */
    protected static $_sessionId = 'ZeroAI_Verify';
     ...
     *        验证码长度
     * @param int $mode
     *        模型 0:数字 1:小写字母 2:大写字母 3:字母与数字组合
     * @param string $type
     *        指定图片类型，一般用默认值.
     * @param int $width
     *        图片宽
    ...
    public static function imgVerify($length = 6, $mode = 3, $type = 'png', $width = 80, $height = 30)
    {
        ...
        $session = ZeroAI::getApplication()->getSession();
    ...
    {
        $session = ZeroAI::getApplication()->getSession();
        $res = $session->get(self::$_sessionId) == md5(strtolower($verifyCode));
        self::cleanVerify();
        return $res;
    }
    ...
    {
        ZeroAI::getApplication()->getSession()->remove(self::$_sessionId);
    }
