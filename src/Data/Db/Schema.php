...
class Schema implements ISchema
{
    ...
    public function __construct(array $policy = [])
    {
        ...
        if (!self::$_driverMap[$schema])
        {
            throw new DbException('实例化数据驱动失败:db.schema:' . $schema . '不存在');
        }
        $this->_policy['schema'] = self::$_driverMap[$schema];
    }
    ...
    public function exec($sql, ...$param)
    {
        ...
        return $this->_getSchema()->query($sql) ? true : false;
    }
    ...
    protected function _parseSqlParam($match, $param)
    {
        ...
        switch($ptype)
        {
            case 't':
                /*解析表名*/
                return $this->_parseParamTable($pval);            
            case 'f':
                /*解析查询字段名*/
                return $this->_parseParamField($pval);              
            case 'u':
                /*解析更新设置*/
                return $this->_parseParamUpdate($pval);
            case 'w':
                /*解析WHERE 条件查询*/
                return $this->_parseParamWhere($pval);
            case 'i':
                /*解析插入语句*/
                return $this->_parseParamInsert($pval);
           default:
               return $pval;
        }
    }
