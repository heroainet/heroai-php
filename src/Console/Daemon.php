 ...
 */
namespace ZeroAI\Console;

use ZeroAI\MVC\ConsoleApplication;
use ZeroAI\Console\Worker\IWorkerHandler;
use ZeroAI\Console\Worker\Base;
...
class Daemon
{
    ...
    const WORKER_DRIVER_MAP = [
        'worker' => '\ZeroAI\Console\Worker\Worker'
    ];
     ...
     * 添加worker实例和对应的进程数目
     *
     * @param \ZeroAI\Console\Worker\Base $worker
     * @param int $num
     */
    public function addWorker(\ZeroAI\Console\Worker\Base $worker)
    {
     ...
     * 获取日志handler
     *
     * @return \ZeroAI\Console\IDaemonHandler
     */
